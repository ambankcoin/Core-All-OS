...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/AMBANKCOINParams"
    else
        PARAMS_DIR="$HOME/.ambankcoin-params"
    fi
fi
...
Failed to fetch the AMBANKCOIN zkSNARK parameters!
...
function main() {
    ...
    cat <<EOF
AMBANKCOIN - fetch-params.sh

This script will fetch the AMBANKCOIN zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common AMBANKCOIN zkSNARK parameters. Note that it is
