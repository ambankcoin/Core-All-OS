...
#include "qt/ambankcoin/splash.h"
#include "qt/ambankcoin/welcomecontentwidget.h"
...
{
    return QCoreApplication::translate("ambankcoin-core", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in ambankcoin.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in ambankcoin.qrc)
...
/** Class encapsulating AMBANKCOIN Core startup and shutdown.
...
/** Main AMBANKCOIN application object */
...
public:
    ...
    /// Get window identifier of QMainWindow (AMBANKCOINGUI)
...
{
    window = new AMBANKCOINGUI(networkStyle, 0);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &AMBANKCOINGUI::detectShutdown);
}
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &AMBANKCOINGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (pwalletMain) {
            ...
            window->addWallet(AMBANKCOINGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(AMBANKCOINGUI::DEFAULT_WALLET);
        ...
        // AMBANKCOIN: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &AMBANKCOINGUI::handlePaymentRequest);
        connect(window, &AMBANKCOINGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(0, "Runaway exception", QObject::tr("A fatal error occurred. AMBANKCOIN can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    Q_INIT_RESOURCE(ambankcoin_locale);
    Q_INIT_RESOURCE(ambankcoin);
    ...
    /// 6. Determine availability of data directory and parse ambankcoin.conf
    /// - Do not call GetDataDir(true) before this step finishes
    if (!fs::is_directory(GetDataDir(false))) {
        QMessageBox::critical(0, QObject::tr("AMBANKCOIN Core"),
    ...
    try {
        ...
        QMessageBox::critical(0, QObject::tr("AMBANKCOIN Core"), QObject::tr("Error: %1").arg(e.what()));
        return 1;
    }
#ifdef ENABLE_WALLET
    ...
    // ambankcoin: links repeatedly have their payment requests routed to this process:
