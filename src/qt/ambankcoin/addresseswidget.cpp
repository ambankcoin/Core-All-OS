...
#include "qt/ambankcoin/forms/ui_addresseswidget.h"
#include "qt/ambankcoin/addresslabelrow.h"
#include "qt/ambankcoin/addnewaddressdialog.h"
#include "qt/ambankcoin/tooltipmenu.h"
...
AddressesWidget::AddressesWidget(AMBANKCOINGUI* parent) :
    PWidget(parent),
...
void AddressesWidget::onStoreContactClicked()
{
    if (walletModel) {
        ...
        auto ambkAdd = Standard::DecodeDestination(address.toUtf8().constData(), isStakingAddress);

        if (!Standard::IsValidDestination(ambkAdd)) {
        ...
        bool isShielded = walletModel->IsShieldedDestination(ambkAdd);
        if (walletModel->updateAddressBookLabels(ambkAdd, label.toUtf8().constData(),
