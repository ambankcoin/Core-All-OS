...
#include "qt/ambankcoin/forms/ui_snackbar.h"
#include "qt/ambankcoin/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(AMBANKCOINGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &AMBANKCOINGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
