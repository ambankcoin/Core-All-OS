...
#include "qt/ambankcoin/dashboardwidget.h"
#include "qt/ambankcoin/forms/ui_dashboardwidget.h"
#include "qt/ambankcoin/sendconfirmdialog.h"
#include "qt/ambankcoin/txrow.h"
...
DashboardWidget::DashboardWidget(AMBANKCOINGUI* parent) :
    PWidget(parent),
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &AMBANKCOINGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    ...
    // update the display unit, to not use the default ("AMBK")
    updateDisplayUnit();
}
...
const QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy()
{
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isAmbk = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakeZAMBK;
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair AMBK, zAMBK
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            ambk = (pair.first != 0) ? pair.first / 100000000 : 0;
            zambk = (pair.second != 0) ? pair.second / 100000000 : 0;
            chartData->totalAmbk += pair.first;
            chartData->totalZambk += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesAmbk.append(ambk);
        chartData->valueszAmbk.append(zambk);

        int max = std::max(ambk, zambk);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->append(chartData->valuesAmbk);
    set1->append(chartData->valueszAmbk);
