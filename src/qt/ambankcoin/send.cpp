...
#include "qt/ambankcoin/send.h"
#include "qt/ambankcoin/forms/ui_send.h"
#include "qt/ambankcoin/addnewcontactdialog.h"
#include "qt/ambankcoin/qtutils.h"
#include "qt/ambankcoin/sendchangeaddressdialog.h"
#include "qt/ambankcoin/optionbutton.h"
#include "qt/ambankcoin/sendconfirmdialog.h"
#include "qt/ambankcoin/guitransactionsutils.h"
#include "qt/ambankcoin/loadingdialog.h"
...
SendWidget::SendWidget(AMBANKCOINGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    coinIcon(new QPushButton())
{
    ...
    setCssProperty(coinIcon, "coin-icon-ambk");
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){onAMBKSelected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){onAMBKSelected(false);});
...
void SendWidget::showHideCheckBoxDelegations(CAmount delegationBalance)
{
    ...
    // coincontrol is not selected, and we are trying to spend transparent AMBKs.
...
void SendWidget::onShieldCoinsClicked()
{
    ...
    if (availableBalance > 0) {
        ...
        ProcessSend(recipients, true, [this](QList<SendCoinsRecipient>& recipients) {
            ...
            if (!res.result) {
                inform(tr("Error generating address to shield AMBKs"));
                return false;
            }
        ...
        inform(tr("You don't have any transparent AMBKs to shield."));
    }
}
...
void SendWidget::onAMBKSelected(bool _isTransparent)
{
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        auto ambkAdd = Standard::DecodeDestination(address.toStdString(), isStakingAddr);

        if (!Standard::IsValidDestination(ambkAdd) || isStakingAddr) {
            inform(tr("Invalid address"));
            return;
        }

        if (walletModel->isMine(ambkAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(ambkAdd, dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
