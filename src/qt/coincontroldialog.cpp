...
void CoinControlDialog::updateLabels()
{
    ...
    ui->labelTitle->setText(fSelectTransparent ?
            "Select AMBK Outputs to Spend" :
            "Select Shielded AMBK to Spend");
    ...
    int nDisplayUnit = BitcoinUnits::AMBK;
                                          ...
                                          const bool isChange)
{
    ...
    // if listMode or change => show AMBANKCOIN address. In tree mode, address is not shown again for direct wallet address outputs
    if (!treeMode) {
