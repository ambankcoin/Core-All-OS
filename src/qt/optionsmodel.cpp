...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::AMBK);
    ...
    if (settings.contains("nAnonymizeAmbankCoinAmount") || reset)
        gArgs.SoftSetArg("-anonymizeambankcoinamount", settings.value("nAnonymizeAmbankCoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in ambankcoin.cpp during shotdown to also remove the window positions
