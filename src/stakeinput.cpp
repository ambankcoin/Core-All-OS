...
CAmbkStake* CAmbkStake::NewAmbkStake(const CTxIn& txin)
{
    if (txin.IsZerocoinSpend()) {
        error("%s: unable to initialize CAmbkStake from zerocoin spend", __func__);
        return nullptr;
    }
    ...
    return new CAmbkStake(txPrev->vout[txin.prevout.n],
                         txin.prevout,
                         pindexFrom);
}

bool CAmbkStake::GetTxOutFrom(CTxOut& out) const
{
    out = outputFrom;
    return true;
}

bool CAmbkStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(outpointFrom.hash, outpointFrom.n);
    return true;
}

CAmount CAmbkStake::GetValue() const
{
    return outputFrom.nValue;
}

bool CAmbkStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal, const bool onlyP2PK)
{
...
CDataStream CAmbkStake::GetUniqueness() const
{
    //The unique identifier for a AMBK stake is the outpoint
...
const CBlockIndex* CAmbkStake::GetIndexFrom() const
{
    // Sanity check, pindexFrom is set on the constructor.
    if (!pindexFrom) throw std::runtime_error("CAmbkStake: uninitialized pindexFrom");
    return pindexFrom;
}

// Verify stake contextual checks
bool CAmbkStake::ContextCheck(int nHeight, uint32_t nTime)
{
