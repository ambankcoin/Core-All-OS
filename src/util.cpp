...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "ambankcoin" is a composite category enabling all AMBANKCOIN-related debug output
            if (ptrCategory->count(std::string("ambankcoin"))) {
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "ambankcoin.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "ambankcoind.pid"));
