...
const char * const AMBANKCOIN_CONF_FILENAME = "ambankcoin.conf";
const char * const AMBANKCOIN_PID_FILENAME = "ambankcoin.pid";
const char * const AMBANKCOIN_MASTERNODE_CONF_FILENAME = "masternode.conf";


// AMBANKCOIN only features
// Masternode
...
#else
    const char* pszModule = "ambankcoin";
#endif
    if (pex)
        return strprintf(
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\AMBANKCOIN
// Windows >= Vista: C:\Users\Username\AppData\Roaming\AMBANKCOIN
// Mac: ~/Library/Application Support/AMBANKCOIN
// Unix: ~/.ambankcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "AMBANKCOIN";
#else
    fs::path pathRet;
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\AMBANKCOINParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\AMBANKCOINParams
    // Mac: ~/Library/Application Support/AMBANKCOINParams
    // Unix: ~/.ambankcoin-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "AMBANKCOINParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectory(pathRet);
    return pathRet / "AMBANKCOINParams";
#else
    // Unix
    return pathRet / ".ambankcoin-params";
#endif
#endif
}
...
void initZKSNARKS()
{
    ...
    if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
        ...
        sapling_spend = "/usr/share/ambankcoin/sapling-spend.params";
        sapling_output = "/usr/share/ambankcoin/sapling-output.params";
        if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
            fParamsFound = true;
        } else {
            // Linux fallback for local installs
            sapling_spend = "/usr/local/share/ambankcoin/sapling-spend.params";
            sapling_output = "/usr/local/share/ambankcoin/sapling-output.params";
        }
#endif
        if (fs::exists(sapling_spend) && fs::exists(sapling_output))
            fParamsFound = true;
...
{
    fs::path pathConfigFile(gArgs.GetArg("-conf", AMBANKCOIN_CONF_FILENAME));
    return AbsPathForConfigVal(pathConfigFile, false);
}

fs::path GetMasternodeConfigFile()
{
    fs::path pathConfigFile(gArgs.GetArg("-mnconf", AMBANKCOIN_MASTERNODE_CONF_FILENAME));
...
void ArgsManager::ReadConfigFile()
{
    ...
    if (!streamConfig.good()) {
        // Create empty ambankcoin.conf if it does not exist
...

    {
        ...
        for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
            // Don't overwrite existing settings so command line settings override ambankcoin.conf
...
fs::path GetPidFile()
{
    fs::path pathPidFile(gArgs.GetArg("-pid", AMBANKCOIN_PID_FILENAME));
