...
#ifndef AMBANKCOIN_STAKEINPUT_H
#define AMBANKCOIN_STAKEINPUT_H
...
public:
    ...
    virtual bool IsZAMBK() const = 0;
    virtual CDataStream GetUniqueness() const = 0;
    virtual bool ContextCheck(int nHeight, uint32_t nTime) = 0;
};


class CAmbkStake : public CStakeInput
{
private:
...
public:
    CAmbkStake(const CTxOut& _from, const COutPoint& _outPointFrom, const CBlockIndex* _pindexFrom) :
            CStakeInput(_pindexFrom), outputFrom(_from), outpointFrom(_outPointFrom) {}

    static CAmbkStake* NewAmbkStake(const CTxIn& txin);
    ...
    bool IsZAMBK() const override { return false; }
    bool ContextCheck(int nHeight, uint32_t nTime) override;
};


#endif //AMBANKCOIN_STAKEINPUT_H
