...
#if defined(NDEBUG)
#error "AMBANKCOIN cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uambk) are considered zero fee (for relaying, mining and transaction creation)
...
CAmount GetBlockValue(int nHeight)
{
    ...
    // Testnet high-inflation blocks [2, 200] with value 250k AMBK
    ...
    } else if (nHeight > 1 && nHeight < 25900000) {    // 6 * 100 months later
        nSubsidy = 386 * COIN;
    } else
        nSubsidy = 0.3 * COIN;
...
{
    util::ThreadRename("ambankcoin-scriptch");
        ...
        if (tx.HasZerocoinSpendInputs()) {
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (isPublicSpend) {
                    ...
                    if (!ZAMBKModule::ParseZerocoinPublicSpend(txIn, tx, state, publicSpend)){
                        return false;
                    }
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    if (Params().GetConsensus().NetworkUpgradeActive(nHeight, Consensus::UPGRADE_POS)) {
        ...
        if (!tx->vout[1].IsZerocoinMint()) {
            ...
            CAmount nMasternodeValue = GetMasternodePayment();
            ...
            if (nHeight != 0 && !IsInitialBlockDownload()) {
                CScript devScriptPubKey = CScript() << ParseHex(Params().GetConsensus().strDevpubkey.c_str()) << OP_CHECKSIG;
...
bool CheckWork(const CBlock& block, const CBlockIndex* const pindexPrev)
{
    ...
    if (block.nBits != nBitsRequired) {
        // AmbankCoin Specific reference to the block with the wrong threshold was used.
        const Consensus::Params& consensus = Params().GetConsensus();
        if ((block.nTime == (uint32_t) consensus.nAmbankCoinBadBlockTime) &&
                (block.nBits == (uint32_t) consensus.nAmbankCoinBadBlockBits)) {
            // accept AMBANKCOIN block minted with incorrect proof of work threshold
            return true;
        }
...
bool AcceptBlock(const CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> ambkInputs;
        std::vector<CTxIn> zAMBKInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
            if(stakeIn.IsZerocoinSpend()){
                zAMBKInputs.push_back(stakeIn);
            }else{
                ambkInputs.push_back(stakeIn);
            }
        }
        const bool hasAMBKInputs = !ambkInputs.empty();
        const bool hasZAMBKInputs = !zAMBKInputs.empty();
        ...
        for (const auto& txIn : block.vtx) {
            ...
            for (const CTxIn& in: tx.vin) {
                if(consensus.NetworkUpgradeActive(nHeight, Consensus::UPGRADE_ZC)) {
                    ...
                    if (isPrivZerocoinSpend || isPublicSpend) {
                        ...
                        if (isPublicSpend) {
                            libzerocoin::ZerocoinParams* params = consensus.Zerocoin_Params(false);
                            PublicCoinSpend publicSpend(params);
                            if (!ZAMBKModule::ParseZerocoinPublicSpend(in, tx, state, publicSpend)){
                                return false;
                            }
                            spend = publicSpend;
                        } else {
                ...
                if(hasAMBKInputs) {
                    ...
                    for (const CTxIn& ambkIn : ambkInputs)
                        if(ambkIn.prevout == in.prevout)
        ...
        if (isBlockFromFork) {
            ...
            // Now that this loop if completed. Check if we have zAMBK inputs.
            if(hasZAMBKInputs) {
                for (const CTxIn& zAmbkInput : zAMBKInputs) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(zAmbkInput);
                    ...
                    if (!ContextualCheckZerocoinSpendNoSerialCheck(stakeTxIn, &spend, pindex->nHeight, UINT256_ZERO))
                        return state.DoS(100,error("%s: forked chain ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                                   stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zambk");
...
int ActiveProtocol()
{
    // SPORK_14 was used for 30000 (v4.1.1), commented out now.
