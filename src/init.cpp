...
void PrepareShutdown()
{
    ...
    util::ThreadRename("ambankcoin-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), AMBANKCOIN_CONF_FILENAME));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), AMBANKCOIN_PID_FILENAME));
#endif
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all AMBANKCOIN specific functionality (Masternodes, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-mnconf=<file>", strprintf(_("Specify masternode configuration file (default: %s)"), AMBANKCOIN_MASTERNODE_CONF_FILENAME));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:34001"));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2015-%i The AMBANKCOIN Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
{
    util::ThreadRename("ambankcoin-loadblk");
...
/** Sanity checks
 *  Ensure that AMBANKCOIN is running in a usable environment with all
...
#endif
    LogPrintf("AMBANKCOIN version %s (%s)\n", version_string, CLIENT_DATE);
}
...
static bool LockDataDirectory(bool probeOnly)
{
    ...
    // Make sure only a single AMBANKCOIN process is using the data directory.
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                //AMBANKCOIN specific: zerocoin and spork DB's
                ...
                // AMBANKCOIN: load previous sessions sporks if we have them.
