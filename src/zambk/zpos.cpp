...
bool CLegacyZAmbkStake::InitFromTxIn(const CTxIn& txin)
{
    // Construct the stakeinput object
    if (!txin.IsZerocoinSpend())
        return error("%s: unable to initialize CLegacyZAmbkStake from non zc-spend", __func__);
    ...
    *this = CLegacyZAmbkStake(spend);

    // Find the pindex with the accumulator checksum
    if (!GetIndexFrom())
        return error("%s : Failed to find the block index for zambk stake origin", __func__);

    // All good
    return true;
}

CLegacyZAmbkStake::CLegacyZAmbkStake(const libzerocoin::CoinSpend& spend) : CStakeInput(nullptr)
{
...
const CBlockIndex* CLegacyZAmbkStake::GetIndexFrom() const
{
...
CAmount CLegacyZAmbkStake::GetValue() const
{
    return denom * COIN;
}

CDataStream CLegacyZAmbkStake::GetUniqueness() const
{
...
bool CLegacyZAmbkStake::ContextCheck(int nHeight, uint32_t nTime)
{
    const Consensus::Params& consensus = Params().GetConsensus();
    if (!consensus.NetworkUpgradeActive(nHeight, Consensus::UPGRADE_ZC_V2) || nHeight >= consensus.height_last_ZC_AccumCheckpoint)
        return error("%s : zAMBK stake block: height %d outside range", __func__, nHeight);
