...
pkgdatadir = $(datadir)/ambankcoin
pkgincludedir = $(includedir)/ambankcoin
pkglibdir = $(libdir)/ambankcoin
pkglibexecdir = $(libexecdir)/ambankcoin
...
CONFIG_HEADER = $(top_builddir)/src/config/ambankcoin-config.h
...
am__DIST_COMMON = $(srcdir)/Makefile.in \
	...
	$(top_srcdir)/src/config/ambankcoin-config.h.in \
...
BITCOIN_CLI_NAME = ambankcoin-cli
BITCOIN_DAEMON_NAME = ambankcoind
BITCOIN_GUI_NAME = ambankcoin-qt
BITCOIN_TX_NAME = ambankcoin-tx
...
OTOOL64 = 
PACKAGE = ambankcoin
PACKAGE_BUGREPORT = https://github.com/ambankcoin/Core-All-OS/issues
...
DIST_CONTRIB = $(top_srcdir)/contrib/ambankcoin-cli.bash-completion \
	       $(top_srcdir)/contrib/ambankcoin-tx.bash-completion \
	       $(top_srcdir)/contrib/ambankcoind.bash-completion \
...
WINDOWS_PACKAGING = $(top_srcdir)/share/pixmaps/ambankcoin.ico \
  $(top_srcdir)/share/pixmaps/nsis-header.bmp \
  $(top_srcdir)/share/pixmaps/nsis-wizard.bmp \
  $(top_srcdir)/doc/README_windows.txt

LINUX_PACKAGING = $(top_srcdir)/share/pixmaps/ambankcoin16.xpm \
  $(top_srcdir)/share/pixmaps/ambankcoin32.xpm \
  $(top_srcdir)/share/pixmaps/ambankcoin128.png
...
COVERAGE_INFO = baseline.info \
  ...
  test_ambankcoin_coverage.info test_ambankcoin.info
...
src/config/ambankcoin-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/ambankcoin-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/ambankcoin-config.h
$(top_srcdir)/src/config/ambankcoin-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/ambankcoin-config.h src/config/stamp-h1
...
$(OSX_APP)/Contents/MacOS/AMBANKCOIN-Qt: all-recursive
	$(MKDIR_P) $(@D)
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/AMBANKCOIN-Qt
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/AMBANKCOIN-Qt: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#test_ambankcoin.info: baseline_filtered.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c $(LCOV_OPTS) -d $(abs_builddir)/src -t test_ambankcoin -o $@
#	$(LCOV) -z $(LCOV_OPTS) -d $(abs_builddir)/src

#test_ambankcoin_filtered.info: test_ambankcoin.info
#	$(abs_builddir)/contrib/filter-lcov.py $(LCOV_FILTER_PATTERN) $< $@
#	$(LCOV) -a $@ $(LCOV_OPTS) -o $@

#functional_test.info: test_ambankcoin_filtered.info
...
#test_ambankcoin_coverage.info: baseline_filtered.info test_ambankcoin_filtered.info
#	$(LCOV) -a $(LCOV_OPTS) baseline_filtered.info -a test_ambankcoin_filtered.info -o $@

#total_coverage.info: test_ambankcoin_filtered.info functional_test_filtered.info
#	$(LCOV) -a $(LCOV_OPTS) baseline_filtered.info -a test_ambankcoin_filtered.info -a functional_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_ambankcoin.coverage/.dirstamp:  test_ambankcoin_coverage.info
...
#cov: test_ambankcoin.coverage/.dirstamp total.coverage/.dirstamp
...
clean-local: clean-docs
	rm -rf coverage_percent.txt test_ambankcoin.coverage/ total.coverage/ test/tmp/ cache/ $(OSX_APP)
