...
class ReorgStakeTest(AmbankCoinTestFramework):
    ...
    def check_money_supply(self, expected_ambk):
        # verify that nodes have the expected AMBK supply
        ambk_supply = [self.nodes[i].getsupplyinfo(True)['transparentsupply']
                      for i in range(self.num_nodes)]
        assert_equal(ambk_supply, [DecimalAmt(expected_ambk)] * self.num_nodes)
    ...
    def run_test(self):
        ...
        # Verify that AMBK supply was properly updated after the reorgs
        self.log.info("Check AMBK supply...")
