AMBANKCOIN Core version *v5.0.0* is now available from:  <https://github.com/ambankcoin-project/ambankcoin/releases>
...
AMBANKCOIN Core v5.0.0 is a mandatory update for all users. This release contains a protocol upgrade (to `70920`), new consensus rules and improvements that are not backwards compatible with older versions. Users will need to update their clients before enforcement of this update goes into effect.
...
#### Sapling Parameters
In order to run, AMBANKCOIN Core now requires two files, `sapling-output.params` and `sapling-spend.params` (with total size ~50 MB), to be saved in a specific location.
...
For the other packages, the user must save the param files in the proper location, before being able to run AMBANKCOIN v5.0.0:
- macOS/Linux `tar.gz` tarballs include a bash script (`install-params.sh`) to copy the parameters in the appropriate location.
- Windows `.zip` users need to manually copy the files from the `share/ambankcoin` folder to the `%APPDATA%\AMBANKCOINParams` directory.
...
#### Overview
Users will be able to protect their financial information by sending and receiving AMBKs privately, as well as sending and receiving encrypted messages attached to the shield transactions.
More visual information about the protocol can be found at https://ambankcoin.com .
...
* Top bar showing shield AMBK balances.
...
* New derivation path for Sapling keys: Shield addresses are derived from the same wallet master seed used for the deterministic derivation of all other keys. The same seed, therefore, can be used to restore both transparent and Shield addresses, recovering both transparent and Shield AMBK balance.
...
See the release notes from [v4.2.0](https://github.com/AMBANKCOIN-Project/AMBANKCOIN/blob/master/doc/release-notes/release-notes-4.2.0.md#label-and-account-apis-for-wallet) for a full description of the changes from the 'account' API to the 'label' API.
...
| `getsupplyinfo` | Returns detailed AMBK supply information | No |
...
* `viewshieldtransaction`
  ...
  Result:
  {
    ...
    "fee"  : x.xxx,             (numeric) The transaction fee in AMBK
    "spends" : [
      {
        ...
        "value" : x.xxx                 (numeric) The amount in AMBK
...
* `getinfo`
  The `zerocoinbalance` JSON return entry and the `zAMBKsupply` return JSON object have been removed due to zerocoin deprecation.
  The `balance` JSON return entry is now all inclusive (the sum of all transparent and shield AMBK)
  The `moneysupply` JSON return entry is the total AMBK supply (the sum of all transparent and shield AMBK)
...
| `getaccountaddress` | Returned the current AMBANKCOIN address associated with an internal account | Account system removal |
| `setaccount` | Associate a AMBANKCOIN address with an internal account name | Account system removal |
| `getaccount` | Returned the internal account associated with a given AMBANKCOIN address | Account system removal |
| `getaddressesbyaccount` | Returned a list of AMBANKCOIN addresses associated with an internal account | Account system removal |
...
| `sendfrom` | Send an amount from an internal account to a AMBANKCOIN address | Account system removal |
...
| `multisend` | Configure sending a portion of stake rewards to a AMBANKCOIN address | Pending a full rewrite |
| `getzerocoinbalance` | Returned the wallet's total zAMBK balance | Zerocoin deprecation |
| `listmintedzerocoins` | List all zAMBK mints in the wallet | Zerocoin deprecation |
| `listzerocoinamounts` | List information about your zerocoin amounts | Zerocoin deprecation |
| `listspentzerocoins` | List all the spent zAMBK mints in the wallet | Zerocoin deprecation |
| `mintzerocoin` | Mint the specified zAMBK amount | Zerocoin deprecation |
| `spendzerocoin` | Spend zAMBK to a AMBK address | Zerocoin deprecation |
| `spendzerocoinmints` | Spend specific zAMBK mints to a AMBK address | Zerocoin deprecation |
...
| `reconsiderzerocoins` | Check archived zAMBKs to see if any mints were added to the chain | Zerocoin deprecation |
| `setzambkseed` | Set the wallet's deterministic zAMBK seed to a specific value | Zerocoin deprecation |
| `getzambkseed` | Return the current deterministic zAMBK seed | Zerocoin deprecation |
| `generatemintlist` | Show mints that are derived from the deterministic zAMBK seed | Zerocoin deprecation |
| `dzambkstate` | Show the current state of the mintpool of the wallet | Zerocoin deprecation |
| `searchdzambk` | Do a search for deterministically generated zAMBK that have not yet added to the wallet | Zerocoin deprecation |
...
### Build Systems
- #1856 `1bbb906059` [CMake] Add test_ambankcoin unit test target (Fuzzbawls)
...
- #2041 `a15dc3417d` [Build] CMake ambankcoind and unit tests: fixing missing MacOS framework. (furszy)
